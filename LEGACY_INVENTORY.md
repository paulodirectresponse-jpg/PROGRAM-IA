...
| ZIP no browser | REMOVED | Upload vai ao importador backend. |
...
| SQLite local | KEEP AS OPERATIONAL CACHE | Sustenta o runtime local; cloud canônico é Supabase normalizado. |
...
| Agent Engine | FEATURE FLAG | State machine básica existe; repair bounded e benchmark real ainda não estão concluídos. |
| `public/` como artefato Cloudflare | REMOVED | Direct Upload não publica `public/` automaticamente; exige build artifact real. |

## Regra para remoções destrutivas

Não remover snapshot antigo ou colunas espelho apenas para limpar schema. Primeiro executar reconciliação real, provar restauração em runtime limpo e só então aplicar migration versionada.
