...
export class ModelRouter {
  ...
  static openCandidateCircuit(id:string,minutes=30,healthState='open'){
    ...
    db.prepare('UPDATE model_candidates SET consecutive_failures=?,health_state=?,circuit_open_until=?,updated_at=? WHERE id=?').run(n,healthState,until,now,id);
  }
  static recordCandidateResult(id:string,ok:boolean,kind?:FailureKind){
    const now=new Date().toISOString();
    if(ok){db.prepare("UPDATE model_candidates SET health_state='healthy',consecutive_failures=0,circuit_open_until=NULL,updated_at=? WHERE id=?").run(now,id);return;}
    const r=db.prepare('SELECT consecutive_failures FROM model_candidates WHERE id=?').get(id) as any;
    const n=(r?.consecutive_failures||0)+1;
    if(kind==='incompatible'){
      const until=new Date(Date.now()+6*60*60*1000).toISOString();
      db.prepare("UPDATE model_candidates SET consecutive_failures=?,health_state='incompatible',circuit_open_until=?,updated_at=? WHERE id=?").run(n,until,now,id);return;
    }
  ...
  static repairFalseBudgetQuarantines(hours=24){
    ...
    const steps=db.prepare(`SELECT s.id step_id,s.context_json,r.user_id FROM agent_steps s JOIN agent_runs r ON r.id=s.run_id WHERE s.created_at>=? AND s.context_json IS NOT NULL`).all(since) as Array<{step_id:string;context_json:string;user_id:string}>;
    let repaired=0;
    for(const step of steps){
      let events:any[]=[];try{const parsed=JSON.parse(step.context_json||'{}');events=Array.isArray(parsed?.events)?parsed.events:[];}catch{continue;}
      for(const event of events){
        ...
        const providerKey=String(event?.providerKey||'').trim(),modelId=String(event?.modelId||'').trim(),profileKey=String(event?.profile||'').trim();
        if(!providerKey||!modelId||!profileKey)continue;
        const invocation=db.prepare('SELECT 1 FROM model_invocations WHERE step_id=? AND provider_key=? AND model_id=? LIMIT 1').get(step.step_id,providerKey,modelId);if(invocation)continue;
        const candidate=db.prepare(`SELECT c.id,c.health_state,c.circuit_open_until FROM model_candidates c JOIN model_profiles p ON p.id=c.profile_id WHERE p.user_id=? AND p.profile_key=? AND c.provider_key=? AND c.model_id=? LIMIT 1`).get(step.user_id,profileKey,providerKey,modelId) as any;
        ...
        db.prepare("UPDATE model_candidates SET health_state='healthy',consecutive_failures=0,circuit_open_until=NULL,updated_at=? WHERE id=?").run(new Date().toISOString(),candidate.id);repaired++;
      }
    }
    return repaired;
  }
  static spent(userId:string,since:string,runId?:string){
    const r=(runId?db.prepare('SELECT COALESCE(SUM(budget_cost_usd),0) total FROM model_invocations WHERE user_id=? AND run_id=?').get(userId,runId):db.prepare('SELECT COALESCE(SUM(budget_cost_usd),0) total FROM model_invocations WHERE user_id=? AND created_at>=?').get(userId,since)) as any;return Number(r?.total||0);
  }
  static recommendedRunBudget(userId:string,mode:string){
    const profiles=this.listProfiles(userId);const profileMax=(key:ProfileKey)=>{const profile=profiles.find((item:any)=>item.profile_key===key&&Number(item.enabled)!==0);if(!profile)return 0;const enabled=(profile.candidates||[]).filter((candidate:any)=>Number(candidate.enabled)!==0);if(!enabled.length)return 0;return Math.max(0,Number(profile.max_cost_usd||0));};
    const routeReserve=Math.max(0.05,profileMax('BASE_FREE')+profileMax('EXPERT_PAID'));const normalized=String(mode||'auto').toLowerCase();const policy:Record<string,{calls:number;floor:number;cap:number}>={plan:{calls:2,floor:.75,cap:1.5},review:{calls:2,floor:.75,cap:1.5},publish:{calls:1,floor:.5,cap:1},build:{calls:4,floor:1,cap:2},auto:{calls:5,floor:1.5,cap:2}};const rule=policy[normalized]||policy.auto;return Math.round(Math.min(rule.cap,Math.max(rule.floor,routeReserve*rule.calls))*1000)/1000;
  }
  static assertBudget(_userId:string,_max:number,_o:{runId?:string;runLimit?:number;dailyLimit?:number}={}){
    // Enforcement intentionally disabled while the real-provider pipeline is stabilized.
    // Cost/token telemetry remains active in recordInvocation; this method must not block execution.
    return;
  }
  static recordInvocation(x:{userId:string;projectId?:string;runId?:string;stepId?:string;agentKey?:string;profileKey?:string;providerKey:string;modelId:string;inputTokens?:number;outputTokens?:number;costUsd?:number;costStatus?:'reported'|'known_zero'|'unknown'|'partial';budgetCostUsd?:number;latencyMs:number;status:string;errorCode?:string;retryIndex?:number;contextPackId?:string;contextScope?:string;projectHash?:string;contextTokens?:number;contextSelectedFiles?:string[];contextOmittedFilesCount?:number;}){
    const knownCost=x.costUsd===undefined||x.costUsd===null?null:Math.max(0,Number(x.costUsd)||0);const costStatus=x.costStatus||(knownCost===null?'unknown':knownCost===0?'known_zero':'reported');const reservation=Math.max(0,Number(x.budgetCostUsd||0));const budgetCost=(costStatus==='unknown'||costStatus==='partial')?Math.max(knownCost||0,reservation):(knownCost||0);
    db.prepare(`INSERT INTO model_invocations(id,user_id,project_id,run_id,step_id,agent_key,profile_key,provider_key,model_id,input_tokens,output_tokens,cost_usd,cost_status,budget_cost_usd,latency_ms,status,error_code,retry_index,context_pack_id,context_scope,project_hash,context_tokens,context_selected_files_json,context_omitted_files_count,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(`inv-${crypto.randomUUID()}`,x.userId,x.projectId||null,x.runId||null,x.stepId||null,x.agentKey||null,x.profileKey||null,x.providerKey,x.modelId,x.inputTokens||0,x.outputTokens||0,knownCost,costStatus,budgetCost,x.latencyMs,x.status,x.errorCode||null,x.retryIndex||0,x.contextPackId||null,x.contextScope||null,x.projectHash||null,x.contextTokens||0,JSON.stringify(x.contextSelectedFiles||[]),x.contextOmittedFilesCount||0,new Date().toISOString());
    if(x.runId){db.prepare('UPDATE agent_runs SET spent_usd=(SELECT COALESCE(SUM(budget_cost_usd),0) FROM model_invocations WHERE user_id=? AND run_id=?) WHERE id=? AND user_id=?').run(x.userId,x.runId,x.runId,x.userId);}
  }
}
