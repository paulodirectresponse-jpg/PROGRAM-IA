...
const OWNED=['providers','skills','integrations','user_secrets','model_profiles'] as const;
...
export class CloudSyncService{
  ...
  static import(userId:string,s:Snapshot){if(s.userId!==userId)throw Error('Snapshot não pertence ao usuário autenticado.');const order=['providers','skills','integrations','user_secrets','model_profiles','model_candidates','projects','project_sources','branches','conversations','messages','checkpoints','verifications','attachments','plans','tasks','agent_runs','agent_steps','tool_executions','model_invocations'];const tx=db.transaction(()=>{for(const name of order){for(const row of s.tables[name]||[]){const cols=Object.keys(row);if(!cols.length)continue;db.prepare(`INSERT OR REPLACE INTO ${name} (${cols.join(',')}) VALUES (${cols.map(()=>'?').join(',')})`).run(...cols.map(c=>row[c]));}}});tx();for(const [projectId,items] of Object.entries(s.files||{})){if(!WorkspaceManager.verifyProjectOwnership(projectId,userId))continue;for(const [name,b64] of Object.entries(items)){WorkspaceManager.writeBinaryFile(projectId,name,Buffer.from(b64,'base64'));}}}
