...
export class CloudSyncService{
  ...
  private static assertSecretsReadable(snapshot:Snapshot){const result=SecretService.validateEncryptedRows(snapshot?.tables?.user_secrets||[]);if(!result.valid)throw Error(`As credenciais remotas não podem ser descriptografadas com a SECRETS_MASTER_KEY atual (${result.failures.join(', ')}).`);}
  ...
  static async bootstrap(userId:string,legacyUserIds:string[]=[]){try{if(!this.configured())return{status:'not_configured',deviceId};let row=await this.remote(userId);let migratedFrom:string|undefined;
    ...
    if(migratedFrom){await this.push(userId);await this.removeRemote(migratedFrom);}
    return{status:'synced',restored:true,revision:row.revision,migratedFrom};
  }
  const hasLocal=(db.prepare('SELECT COUNT(*) n FROM projects WHERE user_id=?').get(userId) as any).n>0;
  return hasLocal?await this.push(userId):{status:'local_only',deviceId};}catch(e:any){return{status:'error',message:e.message,deviceId};}}
