...
export class CloudSyncService{
  ...
  static async pushDirect(userId:string){return SupabasePersistenceService.push(userId,this.firebaseUid(userId),this.export(userId));}
  static async pullDirect(userId:string){const direct=await SupabasePersistenceService.pull(userId,this.firebaseUid(userId));if(direct.status==='synced'&&direct.snapshot){this.assertSecretsReadable(direct.snapshot);this.import(userId,direct.snapshot as Snapshot);if(direct.migratedFrom)await this.pushDirect(userId);return{status:'synced',restored:true,source:'direct',migratedFrom:direct.migratedFrom};}return direct;}
  static async syncAll(userId:string){const direct=await this.pushDirect(userId);const legacy=await this.push(userId);return{...direct,legacyRevision:(legacy as any).revision};}
