...
export class CloudSyncService{
  ...
  private static headers(extra:Record<string,string>={}){const key=this.key();return{apikey:key,Authorization:`Bearer ${key}`,'Content-Type':'application/json',...extra};}
  ...
  static async bootstrap(userId:string){try{const row=await this.remote(userId);if(row){
    ...
    this.import(userId,row.payload);return{status:'restored',revision:row.revision};
  }return await this.push(userId);}catch(e:any){return{status:'error',message:e.message,deviceId};}}
}
